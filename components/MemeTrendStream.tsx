...
import { TrendingUp, MessageSquare, Plus, MousePointer2 } from 'lucide-react';
...
const MemeTrendStream: React.FC<MemeTrendStreamProps> = ({ data }) => {
  ...
  // Find max value to normalize curves and fill the box
  const maxValue = useMemo(() => {
    return Math.max(...data.points.flatMap(p => [p.posts, p.likes, p.comments])) * 1.1;
  }, [data.points]);

  const generateSmoothPath = (key: 'posts' | 'likes' | 'comments', isArea = false) => {
    if (data.points.length === 0) return '';
    
    const points = data.points.map((p, i) => ({
      x: (i / (data.points.length - 1)) * 100,
      y: 100 - (p[key] / maxValue) * 100
    }));
    ...
    <div className="relative mb-40 last:mb-0">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-6 mb-12 group/header cursor-none">
        <div className="flex items-baseline gap-8">
          ...
          <h3 className="text-6xl md:text-8xl font-black text-white tracking-tighter transition-colors group-hover/header:text-[#ff4d00]">
            {data.hashtag}
          </h3>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_450px] gap-16">
        {/* Main Graph Area - Full Width Stretch */}
        <div 
          ref={containerRef}
        ...
        >
          ...
          <div className="absolute top-8 right-10 flex flex-col gap-3 z-10 pointer-events-none">
             <div className="flex items-center justify-end gap-3">
                <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">点赞数 / Likes</span>
                <div className="w-8 h-[2px] bg-[#ff4d00]"></div>
             </div>
             <div className="flex items-center justify-end gap-3">
                <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">评论量 / Comments</span>
                <div className="w-8 h-[1px] bg-white/40 border-t border-dashed border-white/20"></div>
             </div>
             <div className="flex items-center justify-end gap-3">
                <span className="text-[9px] font-black text-white/10 uppercase tracking-widest">发帖数 / Posts</span>
